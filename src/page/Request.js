...
const Request = () => {
  ...
  const handleDeleteRequest = async (requestId) => {
    try {
      await axios.delete(`${process.env.REACT_APP_SERVER_URL}/deleterequest/${requestId}`);
        ...
        <div className="flex justify-center">
          <div className="flex flex-col items-center w-full max-w-8xl">
            {requests.map((request) => (
              <div
                key={request._id}
                className="flex justify-between items-center w-full rounded-lg shadow-md p-4 mb-4"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="font-bold text-sm">{request._id}</h1>
                    <p className="text-sm">Game: {request.name}</p>
                    <p className="text-sm">Status: {request.state}</p>
                  </div>
                </div>
